...
2. **Semantic validation** — *"would a human reasonably put this on their personal calendar?"* `SEMANTIC_REJECTION_PATTERNS` (same file) rejects generic trade-show-presence pieces, partnerships, product reviews, and "ecosystem update" posts even when they mention a real product name — e.g. the real, observed title "HONOR Advances Its AI Vision at MWC 2026 with Robot Phone, Humanoid Robot and Magic V6" mentions "Magic V6" but describes general AI strategy at a trade show, not a release, and is rejected outright.
3. **Canonical event detection.** Even after excluding obvious article-types, two genuinely launch-shaped titles can still describe the *same* real-world event — e.g. a teaser and its regional follow-up, published days apart. [`src/validation/canonicalEventFilter.ts`](src/validation/canonicalEventFilter.ts) groups published events by `company` (falling back to `category`), clusters each group by a rolling 14-day window (chosen over a fixed calendar bucket so it doesn't split or merge events sitting near a boundary), and keeps exactly one event per cluster: **the event should describe the first official announcement only**, so a non-regional title always wins over a same-cluster regional-rollout title ("...in Western Europe"), regardless of verb strength — the one exception being when *every* candidate in the cluster is regionally scoped (matching the principle already established for sources like Vivo, whose only public newsroom is India's: a sole regional source is authoritative for that launch, not a "follow-up"). Within whichever set applies, a title with an explicit launch verb (`Launches`/`Unveils`/`Debuts`/`Announces`) that isn't itself derivative coverage wins, then the earliest by date. Everything else in the cluster is rejected and listed in the build report as a "canonical duplicate."
4. **Canonical titles and descriptions.** Raw newsroom headlines are marketing copy, not calendar entries — "HONOR Launches Magic V6: The Ultimate AI Foldable Flagship Blending Cross-Ecosystem Productivity with Ultra-Slim Durability" becomes **"HONOR Magic V6 Launch"**. `getCanonicalTitle()` in [`src/utils/curatedDescriptions.ts`](src/utils/curatedDescriptions.ts) (alongside the pre-existing `getCuratedDescription()`) rewrites titles for every recognized recurring franchise — Google I/O, WWDC, Microsoft Build, OpenAI DevDay, Samsung Unpacked, the Android release family, and vendor-specific product launches (HONOR Magic/Series, vivo X-series) — into a short, immediately recognizable form, falling back to the original title only when nothing matches. Every source calls both functions the same way `getCuratedDescription` was already being called; no new plumbing.

Verified live: this correctly dropped Samsung to 0 published events (its current newsroom feed's only Unpacked-dated articles were `[Interview]`/`[Infographic]`/`[Invitation]` coverage — never the announcement itself), dropped the MWC "Advances Its AI Vision" piece outright, collapsed Honor's Magic V5 teaser + Western-Europe-rollout pair into one (keeping the non-regional teaser, renamed to "HONOR Magic V5 Launch"), and rewrote every remaining title into its canonical form (`"Announcing OpenAI DevDay 2025"` → `"OpenAI DevDay 2025"`, `"Android 17 is here"` → `"Android 17 Stable Release"`, etc.). All of this is the intended outcome of "reliability over completeness, missing over wrong, quality over quantity" — not bugs.

### Reliability policy: most reliable official source available, no fragile scraping

This project follows **reliability over automation** — but that does *not* mean "no official feed means manual.json only." The actual rule is: use the most reliable **official** source available, in this order of preference: (1) an official dedicated event landing/save-the-date page (`event_page`) — maximizes lead time, since these are updated *ahead of* the event by design, (2) an official RSS/Atom feed, (3) an official newsroom page that's server-rendered and has stable, specific CSS selectors, discovered directly or via a sitemap (`sitemap`), (4) if none of the above exists — the page is client-rendered (JS-only), unreachable, or not phone-specific — fall back to `manual.json` and say so honestly in the source code. Concretely, per vendor:

| Vendor / source file | Discovery method | Official source used | Notes |
|---|---|---|---|
| **Apple** (`apple.ts`) | `event_page` | `https://www.apple.com/apple-events/` **(HTML scrape)** | Apple's own event landing page — server-rendered, no JS required. Replaces the old newsroom-RSS approach, which often didn't publish an article until the event itself. Only emits an event once the hero section carries an explicit date; the common between-keynotes state (a "post-event" recap with vague callout text like "Coming later this year") correctly yields zero events rather than a guess. |
| **Google — Android releases** (`google.ts`) | `newsroom` | `https://android-developers.googleblog.com/atom.xml` | Official Android Developers Blog. Stable/Beta/Feature Drop rollout posts are retrospective, so the post's own publish date is trusted directly as the confirmed release date — no guessing involved. |
| Google — I/O & Made by Google/Pixel (`google.ts`) | `newsroom` (best-effort) | same Android Developers Blog feed | No dedicated official feed with structured forward-looking dates exists for I/O or Made by Google (which is also where Pixel hardware launches — there is no separate Pixel source, it's the same real-world event). Investigated for a dedicated event page: `io.google` exists but is a fully client-rendered web-components app with no date anywhere in server HTML (same failure mode as Xiaomi) — not viable without Playwright. `store.google.com`'s Made by Google category page does carry a real forward-looking date in server HTML but wasn't adopted yet (a general storefront page, not a dedicated event microsite — higher redesign-churn risk). `manual.json` is the reliable path for these today. |
| **Samsung** (`samsung.ts`) | `event_page` | `https://www.samsung.com/us/unpacked/` **(HTML scrape)** | Samsung's own Unpacked reservation page — server-rendered, with a machine-readable countdown widget (`data-end-time` attribute) carrying the confirmed date directly. Replaces Samsung Newsroom's RSS feed, whose Unpacked coverage clustered around/after the event rather than giving advance notice. Verified live: extracted a real date 6 days ahead of the check. |
| **Microsoft Build** (`microsoft.ts`) | `event_page` | `https://build.microsoft.com/` **(HTML scrape)** | Microsoft's own Build event page — server-rendered, confirmed date present in a plain `og:description` meta tag. Replaces mining Microsoft's general corporate press feed for incidental "Build" mentions. |
| OpenAI DevDay (`openai.ts`) | `newsroom` | `https://openai.com/news/rss.xml` | Official OpenAI news feed, filtered for "DevDay". Unlike the other newsroom feeds, OpenAI has historically published a forward "Announcing DevDay {year}" post ahead of the event — sound evidence, but not yet verified end-to-end against a live DevDay cycle in this project. |
| **Honor** (`honor.ts`) | `newsroom` | `https://www.honor.com/global/news/archive/` **(HTML scrape)** | No RSS/Atom feed, but this page is server-rendered with real, stable article markup — scraped via `src/utils/htmlScraper.ts`, filtered to Magic-series phones/foldables and numbered-series flagship launches only (`src/utils/titleFilter.ts` rejects campaigns, awards shows, and minor-device posts even when they say "launched"). |
| **Vivo** (`vivo.ts`) | `newsroom` | `https://vivonewsroom.in/press-releases/` **(HTML scrape)** | vivo's India newsroom — the only public vivo newsroom with real article content found; also server-rendered and scraped the same way, filtered to X-series/X Fold flagship launches. The live feed mixes genuine launches with sales/variant/CSR posts in the same list; the exclude-wins-over-include filter (see below) is what keeps those out. |
| **Oppo** (`oppo.ts`) | `sitemap` | `https://www.oppo.com/en/sitemap.xml` + per-article scrape | OPPO's newsroom *listing* page is a client-rendered shell with no article content, but individual article pages are fully server-rendered — so the sitemap enumerates candidate URLs (filtered by slug keyword first), and each matching article is scraped individually via `src/utils/htmlScraper.ts`'s `scrapeArticle`. Same reliability tier as Honor/Vivo, just discovered a different way. |
...
| OnePlus (`oneplus.ts`) | — | **None found.** | OnePlus does use `/event/{slug}` and `/launch/{slug}` URL patterns (found via sitemap), but every entry found was a stale campaign microsite (a 2022 anniversary page, a page still live from the 2017 OnePlus 5T launch) — no stable, currently-maintained "next launch" URL exists to point at. `manual.json` only. |
...
| `manual.ts` | `manual` | `data/manual.json` | The authoritative source for everything above, and the only source for Nothing, OnePlus, Xiaomi, Realme, and Sony Xperia. Malformed data here throws loudly rather than being silently skipped. |

**Event pages vs. newsroom articles:** a newsroom article is published when (or after) an event happens — that's what a newsroom is for. A dedicated official event page (Apple Events, Microsoft Build, Samsung Unpacked) is a vendor-maintained, evergreen page that exists specifically to drive advance registration, so it gets updated *ahead of* the event with the confirmed date — the source of first choice for a calendar whose whole purpose is advance notice. `DiscoveryMethod` (`newsroom | event_page | sitemap | manual`, on `TechEvent`) makes this distinction a first-class, queryable property of every event rather than an implicit fact buried in each source file.

## Source maturity

Introduced because "reliability > completeness" means it matters not just *which* official source a provider uses, but *how proven* that implementation is. `SourceMaturity` (`readonly maturity?` on `EventSource`, in `src/sources/EventSource.ts`) is an engineering-confidence rating, separate from `DiscoveryMethod` (how a source finds events) and `SourceType` (the per-event output-confidence tier):

- **`production`** — verified against at least one real live event cycle. The mechanism has been checked against genuine data from a real vendor announcement, not just a plausible-looking fixture.
- **`experimental`** — implemented from sound evidence (a confirmed-live page/feed, a real selector) but not yet verified through a complete real-world event cycle.
- **`deprecated`** — known to work but scheduled for replacement, kept running until its replacement is proven.

| Source | Maturity | Why |
|---|---|---|
| `manual.ts` | `production` | Human-curated, not an extraction mechanism — always production. |
| `google.ts` | `production` | Android release branch proven against many real stable/beta/feature-drop releases. The I/O/Made by Google forward-announced sub-path remains best-effort/unverified within this otherwise-production source. |
| `samsung.ts` | `production` | Countdown-widget extraction verified live: produced a genuine forward-looking date (6 days out) directly from Samsung's own page. |
| `microsoft.ts` | `production` | `og:description` extraction verified live against the real Build 2026 date. |
| `honor.ts` | `production` | Selectors and title patterns verified against real observed launch cycles (Magic V6, Magic8 Pro, etc.). |
| `vivo.ts` | `production` | Selectors and title patterns verified against real observed launch cycles (X300 FE, X300 Ultra, etc.). |
| `oppo.ts` | `production` | This exact sitemap-discovery + per-article pipeline was run live against the real sitemap and 38 real article pages during implementation, surfacing and fixing a genuine bug in the process. |
| `apple.ts` | `experimental` | The hero-section selectors are inferred from Apple's own "post-event" class naming, not confirmed against a real pre-event invite — none was live at time of writing, so the pre-event path is unverified. |
| `openai.ts` | `experimental` | Relies on OpenAI's historical pattern of a forward "Announcing DevDay {year}" post — sound evidence, but not yet verified end-to-end against a live DevDay cycle in this project. |
| `nothing.ts`, `oneplus.ts`, `xiaomi.ts`, `realme.ts`, `sony.ts` | *n/a (stub)* | No discovery mechanism exists at all — these always return `[]` by design (see the table above). The maturity scale doesn't apply to a source with nothing to verify; `manual.json` is the only path for these vendors today. |
